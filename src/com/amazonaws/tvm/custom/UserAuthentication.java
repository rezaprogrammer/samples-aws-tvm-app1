...
public class UserAuthentication {
	...
	public boolean registerUser( String username, String password, String uri ) {
		try {
			log.log( Level.INFO, "#1 Checking username and password" );
			if ( this.checkUsernameExists( username ) ) {
				log.log( Level.WARNING, "Exception during registerUser" );
				return false;
			}
			log.log( Level.INFO, "#2 Checking username and password" );
			this.storeUser( username, password, uri );
			boolean ret = this.authenticateUser( username, password, uri );
			log.log( Level.INFO, "#3 Checking username and password: " + ret );
			return ret;
		}
